...
describe('Testing server', () => {
  it('should send 404 error on a bad route', async () => {
    ...
    const response = await request.get('/wadfasdwe').set('Authorization', `Bearer ${response1.body.token}`);
  ...
  it('should send a 404 error when no food is found', async () => {
    ...
    const response1 = await request.post('/signin')
    .auth(obj.username, obj.password);
    const response = await request.get('/error').set('Authorization', `Bearer ${response1.body.token}`);
    expect(response.status).toEqual(404);
  });
